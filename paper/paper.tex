...
\lz{I completely made up the below paragraph...}

The number of calibration exposures needed is linked to the amount of variability in the instrument.  Calibration exposures must span the range of calibration space accessible by the spectrograph, or at least accessed by the science calibrations that require wavelength solutions.  Otherwise, the dimensionality-reduction method will not have the information required to construct a basis vector that spans the entire accessible calibration space.  More observations are always better to help de-noise the calibration data.
