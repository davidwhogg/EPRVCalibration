...
\KwResult{Basis vectors of the low-dimensional calibration space $g_k(\lambda,m)$ and location of exposures in calibration space expressed by amplitudes $a_{n,k}$}
...
\deleted{The endeavor to reach $0.1\,\mps$ radial-velocity precision necessitates improvements on all levels of spectroscopy, from the instrumentation to the data analysis.  Implementing \name, a hierarchical, non-parametric model for wavelength calibration, is a step towards reducing the error contribution from data reduction pipelines.}
