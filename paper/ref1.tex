...
%DIF DEL draft1.tex   Sun Oct 18 21:25:13 2020
%DIF ADD paper.tex    Fri Oct 23 20:57:40 2020
...
%DIF 8c8
%DIF < \documentclass[twocolumn,table,xcolor]{aastex63}
%DIF -------
\documentclass[twocolumn,table,xcolor,trackchanges]{aastex63} %DIF > 
%DIF -------
...
\affil{Max-Planck-Institut f\"ur Astronomie, Königstuhl 17, D-69117 Heidelberg, Germany}
...
\begin{abstract}\noindent%
\Name\ is a non-parametric, hierarchical framework for precision wavelength-calibration of spectrographs.  It is designed with the needs of extreme-precision radial velocity (\eprv) in mind, which require that instruments be calibrated or stabilized to better than $10^{-4}$ pixels.  Instruments vary along only a few dominant degrees of freedom, especially \eprv\ instruments \DIFdelbegin \DIFdel{, which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend feature highly stabilized optical systems and detectors.  \Name\ takes advantage of this property by using all calibration data to construct a low-dimensional representation of all accessible calibration states for an instrument.  \Name\ also takes advantage of \DIFdelbegin \DIFdel{laser-frequency }\DIFdelend \DIFaddbegin \DIFadd{laser frequency }\DIFaddend combs or etalons, which generate a dense set of stable calibration points.  This density permits the use of a non-parametric wavelength solution that can adapt to any instrument or detector oddities better than \DIFdelbegin \DIFdel{any parametric model}\DIFdelend \DIFaddbegin \DIFadd{parametric models}\DIFaddend , such as a polynomial.  We demonstrate the success of this method with data from the \textsl{EXtreme PREcision Spectrograph} (\expres), which \DIFdelbegin \DIFdel{includes }\DIFdelend \DIFaddbegin \DIFadd{uses }\DIFaddend a laser frequency comb.  When wavelengths are assigned to laser comb lines using \name, the RMS of the residuals is about five times lower than wavelengths assigned using polynomial fits to individual exposures.  \DIFdelbegin \DIFdel{In a typical test case, radial-velocity }\DIFdelend \DIFaddbegin \DIFadd{Radial-velocity }\DIFaddend measurements of HD~34411 showed a reduction in RMS scatter over a 10-month time baseline from \DIFdelbegin \DIFdel{$1.22$ }\DIFdelend \DIFaddbegin \DIFadd{$1.17$ }\DIFaddend to $1.05\, \mps$.
\end{abstract}
...
Precise, radial-velocity \DIFdelbegin \DIFdel{(}%DIFDELCMD < \eprv%%%
\DIFdel{) }\DIFdelend programs have been fruitful in finding and characterizing extra-solar planets \citep[e.g.][]{mayor2011, bonfils2013, plavchan2015, butler2017}.  These programs typically make use of spectrographs with resolutions on the order of $50,000-100,000$, which correspond to line widths on the order of $3000\,\mps$.  The state of the art RV precision reached $1\, \mps$ \DIFdelbegin \DIFdel{in 2016.  }\DIFdelend \DIFaddbegin \DIFadd{by 2016 \mbox{%DIFAUXCMD
\citep{fischer2016}}\hspace{0pt}%DIFAUXCMD
.  }\DIFaddend The newest generation of instruments aim  to reach $0.1\,\mps$ precision, the required precision to detect terrestrial worlds\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{fischer2016}}\hspace{0pt}%DIFAUXCMD
...
the $a_{nk}$ are $K$ scalar amplitudes for each exposure $n$, and the $g_k(\lambda,m)$ are basis functions expressing the ``directions'' in calibration space that the spectrograph can depart from the fiducial calibration.  The resultant $x(\lambda,m,n)$, a list of calibration line positions for a given exposure, can be regarded as the calibration state of the spectrograph for that exposure.  When this calibration structure is used to deliver a wavelength solution, $x(\lambda,m,n)$ can be inverted back into $\lambda(x,m,n)$ to recover wavelengths for each detector position $x$ and echelle order $m$ \DIFaddbegin \DIFadd{(see Figure \ref{fig:cartoon})}\DIFaddend .

The challenge is to learn these basis functions, $g_k(\lambda,m)$, from the data and get the $K$ amplitudes, $a_{nk}$, for every exposure $n$.  There are many ways to discern the basis functions.  In this paper, we present one implementation of \DIFdelbegin %DIFDELCMD < \expres%%%
\DIFdelend \DIFaddbegin \DIFadd{\name}\DIFaddend \ using principal component analysis (PCA) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{wiki_pca}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{pearson1901, jolliffe2016}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .  A PCA is justifiable in the limit where \DIFdelbegin \DIFdel{there is }\DIFdelend \DIFaddbegin \DIFadd{exposures have }\DIFaddend very high signal-to-nose ratio, as is usually the case with typical calibration images.  There are many alternatives to PCA for this dimensionality reduction; we return to this point in Section \ref{sec:choices} below.

\subsection{Dimensionality Reduction: De-Noising of Calibration Frames} \label{sec:denoising}
\Name\ will use calibration images to determine 1) the space in which an instrument varies and 2) where in the accessible calibration space the spectrograph existed for each exposure.  For each calibration exposure, $n$, \name\ requires a full list of lines, $(\lambda,m)$ that are expected \DIFaddbegin \DIFadd{to appear }\DIFaddend in each calibration exposure.  Each line is uniquely defined by a combination of echelle order, $m$, and ``true'' or theoretical wavelength, $\lambda$.  There are many strategies for identifying calibration line positions and matching them to their assigned wavelengths; this problem is left out of scope for this work.  
\DIFdelbegin \DIFdel{That is, \name}\DIFdelend \DIFaddbegin 

\DIFadd{\Name}\DIFaddend \ assumes that \DIFdelbegin \DIFdel{this has been done correctly.  We caution, though, that systematic }\DIFdelend \DIFaddbegin \DIFadd{line positions have been identified ``correctly,'' as in the position of a calibration line is determined the same way as the position of a stellar line when extracting RVs.  This also inherently assumes that the calibration lines are not subject to an effect that the science exposures are not, for example differences in charge transfer inefficiency, non-linearities in the PSF, etc.  We caution that systemic }\DIFaddend errors or large uncertainties in fitting line positions \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{easily }\DIFaddend propagate through to \DIFdelbegin \DIFdel{errors or }\DIFdelend biases in the wavelength models returned by \name.  \DIFaddbegin \DIFadd{For more discussion, see Section \ref{sec:discussion}.
}\DIFaddend 

For each exposure, $n$, every line, $(\lambda,m)$, has an associated fitted (measured) detector position, $x(\lambda,m,n)$, for example x-pixel in an 2D extracted echelle order.  Fitted line centers that are missing from an exposure (e.g. because the fit failed due to noise, the line is not in its usual echelle order, etc.) can be assigned a \code{NaN} (hardware not-a-number) for that \DIFdelbegin \DIFdel{echelle order }\DIFdelend \DIFaddbegin \DIFadd{exposure }\DIFaddend instead.  Let there be $P$ lines per exposure.  \Name\ reads in a $N \times P$ matrix of line positions for each of $P$ lines for each of $N$ exposures.
...
\DIFdelbegin %DIFDELCMD < \KwResult{Basis vectors of the low-dimensional calibration space $g_k(\lambda,m)$, location of exposures in calibration space expressed by amplitudes $a_{n,k}$}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \KwResult{Basis vectors of the low-dimensional calibration space $g_k(\lambda,m)$ and location of exposures in calibration space expressed by amplitudes $a_{n,k}$}
\DIFaddend \While{change in missing or outlier line centers $>$ 0.01\%}
{
...
In the implementation of \name\ presented here, the amplitudes of the principal components are interpolated linearly with respect to time.  This is discussed more in Section \ref{sec:choice_avt}.  \DIFdelbegin \DIFdel{These new, linearly interpolated magnitudes, $a_{n\,k}$ for a }\DIFdelend \DIFaddbegin \DIFadd{Let a prime denote values related to a }\DIFaddend science exposure $n'$ \DIFaddbegin \DIFadd{for which we want wavelengths.  We use linearly interpolated magnitudes, $a_{n',k}$ }\DIFaddend at time $t_{n'}$ \DIFdelbegin \DIFdel{, are then used }\DIFdelend to construct the calibration state of the spectrograph for that point in time.  Using interpolated amplitudes, \DIFdelbegin \DIFdel{$a_{n,k}$}\DIFdelend \DIFaddbegin \DIFadd{$a_{n',k}$}\DIFaddend , and the basis vectors, $g_k(\lambda,m)$, returned by the de-noising process, a new set of calibration lines, \DIFdelbegin \DIFdel{$x(\lambda,m,n')$ }\DIFdelend \DIFaddbegin \DIFadd{$x'(\lambda,m,n')$ }\DIFaddend can be constructed for any exposure as formulated in Equation \ref{eq:excl_wsol}.


\subsection{Interpolating a Wavelength Solution} \label{sec:interp_wsol}
From the de-noising step, \name\ can now construct a set of calibration lines, \DIFdelbegin \DIFdel{$x(\lambda,m,n')$ }\DIFdelend \DIFaddbegin \DIFadd{$x'(\lambda,m,n')$ }\DIFaddend for any exposure $n'$.  To construct a wavelength solution, we invert \DIFdelbegin \DIFdel{$x(\lambda,m,n')$ to $\lambda(x,m,n')$ }\DIFdelend \DIFaddbegin \DIFadd{$x'(\lambda,m,n')$ to $\lambda(x',m,n')$ }\DIFaddend by interpolating known wavelengths of the calibration lines over detector position.  For instance, interpolating the known wavelengths vs. line centers onto every integer $x$ will generate wavelengths for each pixel in an echelle order.

After experiments, we found that a cubic-spline interpolation that enforces monotonicity, such as a Piecewise Cubic Hermite Interpolating Polynomial (PCHIP) interpolator, works well for interpolating wavelengths onto pixels.  A cubic spline allows for more flexibility than a parameterized function, while the enforced monotonicity allows the wavelength solution, \DIFdelbegin \DIFdel{$\lambda(x,m,n')$}\DIFdelend \DIFaddbegin \DIFadd{$\lambda(x',m,n')$}\DIFaddend , to be invertible and prevents spurious deviations that may befall a cubic spline.  Choices in interpolation scheme, $K$, and other tests are further discussed in Section \ref{sec:choice_wvp}.
...
\DIFdelbegin %DIFDELCMD < \KwResult{Wavelengths for detector positions $x'(m,n')$ of exposure $n'$ with time $t_{n'}$}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \KwResult{Wavelengths for detector positions $x'(m,n')$ of exposure $n'$ with time $t_{n'}$, where the prime is used to denote values relevant to this new exposure}
\DIFaddend 

Find $a_{n',k}$ by interpolating $a_{n,k}$ with respect to $t_{n'}$\;
\DIFdelbegin \DIFdel{$x(\lambda,m,n') = g_0(\lambda,m) + \sum_{k=1}^K a_{n'k}\,g_k(\lambda,m)$ }\DIFdelend \DIFaddbegin \DIFadd{$x'(\lambda,m,n') = g_0(\lambda,m) + \sum_{k=1}^K a_{n'k}\,g_k(\lambda,m)$ }\DIFaddend where $K=6$\;
\DIFdelbegin %DIFDELCMD < \For{each unique $m$}{
%DIFDELCMD < 	interpolate $\lambda$ with respect to $x(\lambda,m,n')$ onto pixels $x'(m,n')$\;
%DIFDELCMD < 	}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \For{each unique $m$}{
	interpolate $\lambda$ with respect to $x'(\lambda,m,n')$ onto pixels $x'(m,n')$\;
	}
\DIFaddend \caption{Generating Wavelength Solution}
\end{algorithm}

\DIFaddbegin \DIFadd{The implementation of \name\ described here is hosted on GitHub}\footnote{\url{https://www.github.com/lilyling27/excalibur}}\DIFadd{.
}
...
In order to \DIFaddbegin \DIFadd{comfortably }\DIFaddend satisfy the assumption that there exists only low-order variation, which is needed for \name, we used exposures from after the \DIFaddbegin \DIFadd{LFC stabilized following servicing in summer 2019, where the }\DIFaddend photonic crystal fiber \DIFdelbegin \DIFdel{within the LFC was changed in Summer 2019, and the LFC stabilized}\DIFdelend \DIFaddbegin \DIFadd{was replaced and the polarization was changed to shift the wavelength range of the LFC redwards}\DIFaddend .  In the results presented here, we use 1227 LFC exposures and 78 ThAr exposures taken between October 14 and December 18, 2019 on 29 unique nights.
...
\caption{Difference in predicted and theoretical wavelengths for the wavelength calibration tests described in Section \ref{sec:tests}.  The per line RMS as defined in Equation \ref{eq:rms} is given in the top-right corner in each method's corresponding color.  \DIFdelbeginFL \DIFdelFL{\Name\ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Incorporating denoising }\DIFaddendFL returns the smallest spread in residuals.}
\label{fig:testHists}
\end{figure} 
...
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.455\textwidth]{Figures/lineResids2D_col.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Figures/lineResids2D.pdf}
\DIFaddendFL \caption{Residuals of a single LFC exposure plotted with respect to detector position (as defined by echelle order and x-pixel) for parametric \DIFaddbeginFL \DIFaddFL{(top) }\DIFaddendFL and non-parametric \DIFaddbeginFL \DIFaddFL{(bottom) }\DIFaddendFL wavelength calibration methods.  Each line is colored by the difference between the predicted wavelength and the theoretical wavelength for each line, given in units of $\mps$.  \DIFdelbeginFL \textbf{\DIFdelFL{Top}}%DIFAUXCMD
\DIFdelFL{: residuals to a classic, polynomial fit.  }\textbf{\DIFdelFL{Bottom}}%DIFAUXCMD
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:waveResids}
...
\DIFdelbegin \DIFdel{Figure \ref{fig:waveResids} }\DIFdelend \DIFaddbegin \begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{Figures/lineDensity.pdf}
\caption{\DIFaddFL{Residuals when using ThAr lines to predict wavelengths for LFC lines.  }\textbf{\DIFaddFL{Left}}\DIFaddFL{: residuals for a single LFC exposure plotted with respect to detector position and colored by residual, as in Figure \ref{fig:resid2d}.  The positions of ThAr lines are over-plotted in yellow.  In general, residuals are greater between ThAr lines with greater separation.
}\textbf{\DIFaddFL{Right}}\DIFaddFL{: Comparison of polynomial and interpolated wavelength solutions using either just ThAr lines or LFC lines for a subset of echelle order 94.  The shape of the residuals from a polynomial fit are similar whether using ThAr lines or LFC lines.  A PCHIP interpolated wavelength model guided by LFC lines returns the smallest residuals.}}
\label{fig:waveResids}
\end{figure*}

\DIFadd{Figure \ref{fig:waveResids} (right) }\DIFaddend plots the residuals for a subset of order 94 for both a polynomial-based method and a PCHIP-based method guided by either ThAr lines or LFC lines.   The PCHIP model with ThAr lines (orange, dashed curve) returns huge residuals between two widely-separated ThAr lines that extends out of frame.  The classic, polynomial fit exhibits similar residuals in both amplitude and shape regardless of whether the set of ThAr lines or LFC lines are used.  An interpolated wavelength solution using LFC lines (black, solid curve) exhibits the lowest residuals.
...
\begin{figure*}[t]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{Figures/rvs.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Figures/dm9_rvs.pdf}
\DIFaddendFL \caption{HD 34411 RVs measured with \expres.  \DIFdelbeginFL \DIFdelFL{RVs derived using the implementation of \name\ presented }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{We zoom }\DIFaddendFL in \DIFdelbeginFL \DIFdelFL{this paper }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on all but 8 of the exposures, which were taken over 150 days later.  All RVs }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{plotted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{shown }\DIFaddendFL in blue\DIFdelbeginFL \DIFdelFL{.  The nightly-binned }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; nightly binned }\DIFaddendFL RVs are over-plotted as orange squares.  The RMS of the data set and the RMS of the binned values \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{are }\DIFaddendFL given in the \DIFdelbeginFL \DIFdelFL{top-right }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{top-left }\DIFaddendFL corner.  \DIFaddbeginFL \textbf{\DIFaddFL{Left}}\DIFaddFL{: RVs derived using a polynomial-based wavelength solution.  }\textbf{\DIFaddFL{Right}}\DIFaddFL{: RVs derived using wavelengths from the implementation of \name\ presented in this paper.}\DIFaddendFL }
\label{fig:rvs}
\end{figure*}

We tested \name -generated wavelengths on RV measurements using \expres\ observations of \target, which are presented in Table \ref{tab:rv}.  \target\ is a G0V star.  It is $4.8\, Gyr$ old and relatively quiet ($log R'_{HK} = -5.09$) \citep{brewer2016}.  \DIFaddbegin \DIFadd{Because \target\ has no known planets and should have a smaller contribution from stellar signals, it is a good star to test the effects of the wavelength calibration on the RMS of the returned RVs.  }\DIFaddend We use 114 observations of \target\ taken between October 8, 2019 and \DIFdelbegin \DIFdel{August 13}\DIFdelend \DIFaddbegin \DIFadd{March 5}\DIFaddend , 2020 with SNR 250.  Radial-velocity measurements were derived using a chunk-by-chunk, forward-modeling algorithm ran by the \expres\ team \citep{petersburg2020}.

Figure \ref{fig:rvs} compares the resultant RVs when using a classic, ninth-degree polynomial wavelength solution and an \name -generated wavelength model.  Using \name -generated wavelengths reduces the RMS of the entire data set from \DIFdelbegin \DIFdel{$1.22\, \mps$ }\DIFdelend \DIFaddbegin \DIFadd{$1.17\, \mps$ }\DIFaddend with the classic wavelength solution to $1.05\, \mps$.  This is equivalent to removing an independent, additive noise component of \DIFdelbegin \DIFdel{$0.62\, \mps \,(=\sqrt{1.22^2-1.05^2})$.
A reduction in RMS was observed universally in all data sets tested; }\DIFdelend \DIFaddbegin \DIFadd{$0.52\, \mps \,(=\sqrt{1.17^2-1.05^2})$.
}

\DIFadd{We conducted a direct test of a classically-generated wavelength solution with \name -wavelengths on four }\DIFaddend other data sets\DIFaddbegin \DIFadd{.  All targets }\DIFaddend showed a reduction in \DIFdelbegin \DIFdel{RMS between $0.2-0.5\, \mps$.  }\DIFdelend \DIFaddbegin \DIFadd{or comparable RV RMS.  The results from these data sets can not be interpreted as directly as with \target, though, due to larger contributions from stellar variability, known planets, etc.  As completely mitigating these different effects is out of scope for this paper, we focus here on the results with \target.
}\DIFaddend 
...
\startdata
 ...
 \DIFaddbegin \DIFadd{18771.4196 }\DIFaddend & \DIFaddbegin \DIFadd{2.688 }&	\DIFadd{0.357 }\\
          & \DIFaddend \(\vdots\)  & \\
\enddata
...
For the dimensionality reduction and denoising, the PCA could be replaced by a probabilistic PCA model or other probabilistic linear reduction methods, such as heteroscedastic matrix factorization (HMF)\citep{tsalmantza2012}.  It is also possible to move to non-linear reduction methods, like \DIFaddbegin \DIFadd{a Guassian process latent-variable model, }\DIFaddend an auto-encoder\DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend a normalizing flow \citep[e.g.][]{kramer1991,woodbridge2020}\DIFaddbegin \DIFadd{.  Using a non-linear denoising model could enable \name\ to capture large-scale changes as well as small variations in calibration state.
}\DIFaddend 
...
\caption{\textbf{Top}:  The fiducial calibration of the spectrograph, i.e. the mean line positions for each line throughout the epoch of stability.  \DIFdelbeginFL \DIFdelFL{The following $3 \times 3$ grid of plots }\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{The following $3 \times 3$ grid of plots}} \DIFaddendFL show the first nine principal components constructed using LFC lines.  These principal components represent the basis vectors along which the calibration of the spectrograph can deviate from the fiducial calibration.  For each principal component, or basis vector, each calibration line is plotted according to its echelle order and x-pixel and colored by the value of the basis vector for that line.  Principal components beyond the sixth one become steadily more dominated by noise.}
\label{fig:pcLfc}
\end{figure*}
...
The choice in interpolation method can help guide how many wavelength calibration images are truly needed.  It is unnecessary to take calibration images at times where the same information can be \DIFdelbegin \DIFdel{constructed }\DIFdelend \DIFaddbegin \DIFadd{reconstructed at the desired precision }\DIFaddend by a well-chosen interpolation scheme.  For example, with the \expres\ data shown here, it is clear that nightly calibration images are needed, but for a linear trend, only two calibration images throughout the night are strictly required.

We also tested an implementation of \name\ where the $K$ principal components within a night were fit to a cubic \DIFaddbegin \DIFadd{with respect to time rather than linearly interpolated}\DIFaddend .  This emulates the current, polynomial-based wavelength solution implemented in the \expres\ \DIFdelbegin \DIFdel{pipline}\DIFdelend \DIFaddbegin \DIFadd{pipeline}\DIFaddend , where polynomial fits to calibration files are interpolated to science exposure by fitting \DIFdelbegin \DIFdel{the polynomial coefficients to }\DIFdelend \DIFaddbegin \DIFadd{polynomial coefficients with respect to time to }\DIFaddend a cubic.  We found that \DIFdelbegin \DIFdel{this method }\DIFdelend \DIFaddbegin \DIFadd{using a cubic in place of linear interpolation }\DIFaddend returns comparable RV RMS for most targets, though appears to do better when a night has sparse calibration data.  \DIFaddbegin \DIFadd{This suggests that the nightly behavior of }\expres\DIFadd{\ with respect to time is well described by a cubic function, but LFC exposures are typically taken with enough frequency that a linear interpolation provides a good approximation (see Figure \ref{fig:nightlyVariation})
}\DIFaddend 

The amplitudes $a_{n,k}$ can also be interpolated with respect to any good housekeeping data, not just time.  Best results will come from interpolating with respect to whatever is most strongly correlated with the calibration state of the spectrograph.  For example, with \expres, which is passively temperature controlled, the returned amplitudes $a_{n,k}$ were extremely correlated with the optical bench temperature, as shown in \DIFaddbegin \DIFadd{the top-left plot of }\DIFaddend Figure \ref{fig:nightlyVariation}, suggesting it would also be possible to interpolate the amplitudes with respect to temperature.
...
\caption{Residuals from different interpolation schemes over pixels in echelle order 100.  ThAr lines, shown as blue circles, are used to construct a wavelength solution that is then evaluated at each LFC line, shown as blue \DIFaddbeginFL \DIFaddFL{vertical }\DIFaddendFL lines.  The residuals of each wavelength solution for a subset of the order is shown on the left.  Histograms of the residuals for each method for the complete order is shown on the right.  \DIFdelbeginFL \DIFdelFL{Here, the y-axis is zoomed in to show the shape of each histogram more clearly.  }\DIFdelendFL Note: there is a blended ThAr line at approximately pixel 5300, the \DIFdelbeginFL \DIFdelFL{last }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{right most }\DIFaddendFL ThAr \DIFdelbeginFL \DIFdelFL{line }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lines }\DIFaddendFL plotted.}
\label{fig:xinterp}
\end{figure*} 
...
A simple linear interpolation would give erroneously low values everywhere.  Due to the dispersion intrinsic to echelle spectrographs, the wavelength change between pixels grows greater with greater wavelengths\DIFaddbegin \DIFadd{, even within an order}\DIFaddend .  This means that the function of wavelength vs. pixel across an echelle order will always be monotonically increasing and concave down everywhere.

\DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{Though less of an issue with LFC lines, a }\DIFaddend more classic cubic spline interpolation can run into issues with arc lines, which are irregularly spaced or even blended.  Close lines appearing at virtually the same pixel location but with different wavelengths could coerce a cubic spline into a very high slope\DIFdelbegin \DIFdel{, as with the green curve }\DIFdelend \DIFaddbegin \DIFadd{.  This is demonstrated by the green line }\DIFaddend in Figure \ref{fig:xinterp}\DIFaddbegin \DIFadd{, which shows the results of interpolating between ThAr lines rather than LFC lines}\DIFaddend .  A line blend appears at approximately pixel 5300, causing the spline to twist to nearly vertical to account for both points.  This leads to huge deviations from the correct wavelengths around this line blend as the extraneously high slope of the spline is accounted for.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.45\textwidth]{Figures/lfcDensityTest.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
Similarly, PCA is just one of many possible dimensionality-reduction methods.  We chose to implement \name\ using PCA here for simplicity and computational tractability.  \DIFaddbegin \DIFadd{PCA is a good option because the instrument changes here are small enough that a linear model is an approrpiate representation of the changes.  }\DIFaddend If \name\ was updated to a full probabilistic model, the PCA along with the interpolation model would have to be upgraded to something with better probabilistic properties.  \DIFaddbegin \DIFadd{Other, nonlinear denoising methods may be more robust to large changes, allowing all calibration images ever taken with an instrument to be used to construct the accessible calibration state regardless of hardware adjustments.  }\DIFaddend Further discussion of other implementations of \name\ can be found in Section \ref{sec:choices}.
...
Once we have defined a calibration space that captures all possible degrees of freedom for a stabilized spectrograph, there are many options for pinpointing where the spectrograph is located within that calibration space.  Good housekeeping data, such as temperature or pressure could be used in addition to or instead of time (as mentioned in Section \ref{sec:choice_avt}).  Telemetry that is seen to be correlated with the calibration state of the spectrograph can even be added to the data used to construct the low-dimensional basis.  Furthermore, all exposures taken with the spectrograph in principle contains information about the calibration state of the spectrograph.  Theoretically, tellurics, lines in science exposures, or just the trace positions themselves could also be used to determine an instrument's calibration state, \DIFdelbegin \DIFdel{negating the need for any additional wavelength-calibration source beyond defining the calibration space}\DIFdelend \DIFaddbegin \DIFadd{thereby providing free simultaneous calibration information}\DIFaddend .
